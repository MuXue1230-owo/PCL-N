...
{
    public static IReadOnlyList<SettingsCatalogPage> GlobalPages { get; } = Pages("general:通用|appearance:外观|game:游戏|java:Java|network:下载与网络|storage:存储与迁移|cloud:N Cloud|privacy:隐私与诊断|advanced:更新与高级");
    ...
    public static IReadOnlyList<SettingsCatalogPage> InstanceSettingsSections { get; } = Pages("basic:基本|java:Java|resources:资源与性能|window:窗口与启动|hooks:JVM 与 Hooks|profiles:启动配置|servers:服务器|security:更新与安全|sync:同步与备份|advanced:高级");
    ...
    {
        using var stream = typeof(SettingsCatalog).Assembly.GetManifestResourceStream("PCL.Services.Settings.SettingsCatalog.json")
